...
@section('container')
    ...
    <div class="col-lg-8">
        <form method="POST" action="/dashboard/postingan/{{ $postingan->id }}" enctype="multipart/form-data">
            @method('put')
            @csrf
            <div class="mb-3">
                <label for="title" class="form-label">Judul</label>
                <input type="text" class="form-control @error('title') is-invalid @enderror" id="title" name="title"
                    required autofocus value="{{ old('title', $postingan->title) }}">
                @error('title')
            ...
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('slug') is-invalid @enderror" id="slug" name="slug"
                    readonly required value="{{ old('slug', $postingan->slug) }}">
                @error('slug')
                    <div class="invalid-feedback">
                        {{ $message }}
                    </div>
            ...
            <div class="mb-3">
                ...
                <input type="hidden" name="oldImage" value="{{ $postingan->image }}">
                @if ($postingan->image)
                    <img src="{{ asset('storage/' . $postingan->image) }}"
